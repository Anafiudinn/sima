...
<div class="space-y-6">
    <div class="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
        ...
        {{-- Toolbar untuk viewer --}}
        <div class="p-4 bg-gray-50 border-b border-gray-200">
            <div class="flex flex-wrap items-center gap-4 justify-between">
                {{-- Kontrol Zoom dan Rotasi --}}
                <div class="flex items-center gap-3">
                    <div class="flex items-center gap-2">
                        <button id="zoom-out" title="Perkecil Tampilan"
                            class="p-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
                        ...
                        <span id="zoom-level"
                            class="text-sm font-medium text-gray-800 min-w-[4rem] text-center">100%</span>

                        <button id="zoom-in" title="Perbesar Tampilan"
                            class="p-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                    ...
                    <button id="rotate" title="Putar Tampilan"
                        class="flex items-center gap-2 px-3 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <button id="fullscreen" title="Tampilan Layar Penuh"
                        class="flex items-center gap-2 px-3 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors duration-200">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="flex items-center gap-4 text-sm text-gray-600">
                    ...
                    @endphp
                    <span
                        class="px-2 py-1 {{ $badgeColor[$extension] ?? 'bg-gray-100 text-gray-600' }} rounded-full text-xs font-medium">{{ strtoupper($extension) }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    @php
        ...
        // Gunakan Google Docs Viewer untuk file non-PDF
    ...
    <div id="viewer-container" class="bg-white rounded-xl shadow-lg border border-gray-200 p-4">
        <div class="bg-gray-100 rounded-xl border border-gray-300 overflow-hidden" style="height: 80vh; min-height: 600px;">
            <iframe id="document-viewer" src="{{ $viewerUrl }}"
        ...
        <div class="flex items-start gap-3">
            ...
            <div>
                ...
                <p class="text-blue-600 text-sm mt-1">
                    Untuk dokumen **PDF**, gunakan kontrol zoom dan putar di atas. Untuk dokumen lainnya, gunakan kontrol bawaan dari viewer.
                </p>
            </div>
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const isPDF = @json($isPdf); // Gunakan variabel PHP untuk menentukan tipe file
