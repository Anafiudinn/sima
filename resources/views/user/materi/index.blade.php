...
@section('content')
    <div class="bg-white rounded-xl shadow-lg p-6 mb-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-8 text-sm">
            <div class="flex items-center space-x-2">
               Nama: <span class="font-semibold ml-1">{{ $user->name }}</span>
            </div>
        ...
        <form action="{{ route('user.materi.index') }}" method="GET">
            <div class="flex flex-col lg:flex-row gap-4">
                <div class="relative flex-1">
                    ...
                    <input type="text" name="search" placeholder="Cari materi berdasarkan judul atau deskripsi..."
    ...
    @elseif ($materis->isEmpty())
        <div class="bg-white rounded-xl shadow-lg p-12 text-center">
            <p class="text-gray-500 text-lg">Materi dengan kata kunci "<strong
                    class="text-gray-800">{{ request('search') }}</strong>" tidak ditemukan.</p>
        </div>
    @else
                ...
                <div class="bg-white rounded-xl shadow-md border border-gray-200 p-6 flex flex-col justify-between">
                    <div>
                        <div class="flex items-center justify-between mb-4">
                            ...
                            @endphp
                            <span
                                class="px-2 py-1 {{ $badgeColor[$extension] ?? 'bg-gray-100 text-gray-600' }} rounded-full text-xs font-medium uppercase">{{ $extension }}</span>
                        </div>
                        <p class="text-gray-600 text-sm mb-4 line-clamp-3">
                            {{ $materi->deskripsi ?? 'Tidak ada deskripsi tersedia untuk materi ini.' }}</p>
                    </div>
                    <div class="mt-4 flex items-center justify-between pt-4 border-t border-gray-100">
                        ...
                        <div class="flex gap-2">
                            <a href="{{ route('user.materi.view', $materi->id) }}"
                                class="p-2 text-sm font-medium rounded-md text-blue-600 bg-blue-100 hover:bg-blue-200 transition-colors duration-200"
                                title="Lihat">
                            ...
                            <a href="{{ route('user.materi.download', $materi->id) }}"
                                class="p-2 text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 transition-colors duration-200"
                                title="Download">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
