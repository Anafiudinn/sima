...
@section('content')
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
        <h2 class="text-xl font-semibold text-gray-800 mb-6">Edit Tempat</h2>
        ...
        <form action="{{ route('admin.tempat.update', $tempat->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nama_tempat" id="nama_tempat" value="{{ $tempat->nama_tempat }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="divisi_id" id="divisi_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                    @foreach($divisis as $divisi)
                        <option value="{{ $divisi->id }}" {{ $tempat->divisi_id == $divisi->id ? 'selected' : '' }}>
                            {{ $divisi->nama_divisi }}
                        </option>
                    @endforeach
                </select>
